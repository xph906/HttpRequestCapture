...
function HttpFoxDBService(){
	this.init();
};
HttpFoxDBService.prototype = {
	///Users/a/Library/Application Support//Firefox/Profiles/qfhr7urf.dev/httpfox.sqlite
	FILE_NAME : "httpfox.sqlite",
	ASYNC_INTERVAL_MS : 1000,
	MAX_ASYNC_QUEUE : 1000,
	asyncQueue : [],
	...
	init : function(){
		...
		// Create Tables
		var create_req_table =  "CREATE TABLE http_requests("+"\n"+
        ...
        var create_header_table =  "CREATE TABLE http_request_headers("+"\n"+
        					...
        					"name TEXT,"+"\n" +
        					"value TEXT);";
        					//" UNIQUE (http_request_id, name) ON CONFLICT REPLACE);";
        this.executeSQL(create_header_table,false);
        var create_page_table =  "CREATE TABLE pages("+"\n"+
       ...
       var create_redirect_table = "CREATE TABLE meta_redirects ("+"\n"+
		...
		this.timerCallback.db = this;
		this.timer.initWithCallback(this.timerCallback,this.ASYNC_INTERVAL_MS, Components.interfaces.nsITimer.TYPE_REPEATING_SLACK);
	},

	close : function() {
		if(this.dbConnection) {
			if(this.asyncCallsMade)
				this.dbConnection.asyncClose();
			else
				this.dbConnection.close();
		}
		this.dbConnection = null;
		this.asyncCallsMade = false;
		this.timer.cancel();
	},
	...
	//	dump("executeSQL: "+statement+"\n");
		try {
			if(!async)
				...
				this.asyncCallsMade = true;
				this.asyncQueue.push(this.dbConnection.createAsyncStatement(statement));
				if(this.asyncQueue.length >= this.MAX_ASYNC_QUEUE)
					this.processAsyncQueue();
			}
		}
		catch(error) {
			dump("Logging error: in " + statement+"\n");
			dump(error+"\n");
		}
	},

	processAsyncQueue : function(){
		//dump("in Processing Async Queue "+this.asyncQueue.length+"\n");
		if(this.asyncQueue.length > 0) {
			try{
				this.dbConnection.executeAsync(this.asyncQueue, this.asyncQueue.length);
			}catch(e){
				dump("Logging error: in processAsyncQueue \n");
				dump(e+"\n");
			}
			
		//	dump("processAsyncQueue: After executingAsync DB connection\n");
			this.asyncQueue = [];
		}
	},
	...
	createInsert : function(table, update) {
		...
		//dump("In createInsert: "+statement+"\n");
		return statement;
	},

	timerCallback : {
  		notify: function(timer) {
    		this.db.processAsyncQueue();
  		}
	}
...
HttpFoxService.prototype = 
{
	...
	init: function() 
	{
		...
		try{
			this.DataBase = new HttpFoxDBService();
		}catch(e){
	...
	dumpRequest : function(request, log){
		return ;
		//var x= this.Observer.pageIDFromHttpChannel(request.HttpChannel);
		
		dump(log+": ["+request.id+"]"+request.HttpChannel.requestMethod+": "+request.HttpChannel.URI.spec+"\n");

		dump("State : "+request.EventSource +"\n");
		//dump("Window: "+x+"\n");
		request.HttpChannel.visitRequestHeaders({visitHeader: function(name, value) {
			dump("  "+name+"->"+value+"\n");
			
		}});
		if (request.PostDataHeaders != null)
		{
			dump("  PostDataHeaders: "+request.PostDataHeaders);
		}

		if (request.PostData != null)
		{
			dump("  PostData: "+request.PostData);
		}

		if (request.PostDataParameters != null)
		{
			dump("  PostDataParameters: "+request.PostDataParameters);
		}
		
		if(request.isEffectivePageInfo()){
			...
			dump("No Page Info: ");
			dump("  PageID1: "+request.PageInfo.pageID+
			 " ParentID:"+request.PageInfo.parentID+
			 " location:"+request.PageInfo.location+"\n");
		}
	...
	{
		//dump("addNewRequest1 \n");
		var request = new HttpFoxRequest(requestEvent.HttpFox, requestEvent.HttpChannel, requestEvent.Context, requestEvent, this.DataBase);
		//dump("addNewRequest2 \n");
		//http_request_count++;
		this.Requests.push(request);
		this.PendingRequests.push(request);
		
		//if(requestEvent.isEffectivePageInfo()){
		//	dump("Add request page info\n");
		var newPage = new HttpFoxPage(requestEvent.PageInfo.pageID, 
									requestEvent.PageInfo.parentID,
									requestEvent.PageInfo.location);
	...
	updateRequest: function(index, updatedRequest)
	{
		...
		this.dumpRequest(this.Requests[index],"OldRequest");
		this.Requests[index].StoreHeaderToDB();
	...
	handleRequestEvent: function(requestEvent)
	{
		...
		//this.getWindow(requestEvent.HttpChannel);
		...
		{
			//dump("Error in handleRequest: "+ex+"\n");
			return;
		}
...
function HttpFoxPage(pageID,parentID,location)
{
	this.pageID = pageID;
	this.parentID = parentID;
	this.location = location;
	//dump("HttpFoxPage: "+location+"\n");
};
HttpFoxPage.prototype = {
	pageID : null,
	parentID : null,
	location : null
};
...
// HttpFoxRequest
function HttpFoxRequest(HttpFoxServiceReference, HttpChannelReference, HttpFoxContext, HttpFoxRequestEventReference,db)
{
	//dump("Init Request1 "+this.DBService+"\n" );
	try 
	{
...
HttpFoxRequest.prototype = 
{
	...
	init: function(requestEvent)
	{
		...
		this.PageInfo = new HttpFoxPage(-1,-1,"");

		//XIANG_CODE
		this.id = http_request_count;

		http_request_count++;

		this.StoredKeys = {};
		UselessHeaders = {};
		UselessHeaders["connection"] = true;
		UselessHeaders["content-length"] = true;
		UselessHeaders["content-type"] = true;
		UselessHeaders["cache-control"] = true;
	...
	//called in addNewRequest, AFTER setting page
	StoreRequestToDB : function(){
		this.StoreHeaderToDB();
		var update = {};
		update["id"] = this.id;
		update["url"] = this.DBService.escapeString(this.Url.toLowerCase());
		update["method"] = this.DBService.escapeString(this.RequestMethod.toLowerCase());
		var referer = "referer";
		if(referer in this.StoredKeys){
			update["referrer"] = this.DBService.escapeString(this.StoredKeys[referer]);
		}
		else{
	...
	/*	dump("Store Request :");
		for(var item in update){
			dump(" ["+item+": "+update[item]+"] ");
		}
		dump("\n")
	*/	
		var statement = this.DBService.createInsert("http_requests",update);
		this.DBService.executeSQL(statement, true);

	},

	//called in StoreRequestToDB, addNewRequest and updateRequest
	StoreHeaderToDB : function(){
		//dump("Store Information To DB "+this+"\n");
		tmp = {};
		this.HttpChannel.visitRequestHeaders({visitHeader: function(name, value) {
			tmp[name.toLowerCase()] = value.toLowerCase();
			//dump(" StoreHeaderToDB "+this+"\n");
		}});
		for(var item in tmp){
			if(!(item in this.StoredKeys)){
				this.StoredKeys[item] = tmp[item];
				//dump("New Headers: "+item+"->"+tmp[item]+"\n");
				//Store new header to DB
				this.StoreNewHeaderToDB(item, tmp[item]);
			}
		}
		if (this.PostDataHeaders != null)
		{
		...
		if (this.PostDataParameters != null)
		{
	...
	StoreNewHeaderToDB : function(name,value){
		if(name in UselessHeaders)
			return;
		var update = {};
		//dump("StoreNewHeaderToDB: "+this.id+" || "+name+" "+value+"\n");
		update["http_request_id"] = this.id;
		update["name"] = this.DBService.escapeString(name);
		update["value"] = this.DBService.escapeString(value);
		var statement = this.DBService.createInsert("http_request_headers",update);
		this.DBService.executeSQL(statement, true);

		//filter those useless headers
	},
	
	//called in setPageInfo
	StorePageToDB : function(page){
		if(page.pageID in page_info){
			return;
		}
		else{
			page_info[page.pageID] = page.location;
			//var orginal_id = this.
			var update = {};
			update["id"] = page.pageID;
			update["location"] = this.DBService.escapeString(page.location);
			update["parent_id"] = page.parentID;
			var statement = this.DBService.createInsert("pages",update);
			this.DBService.executeSQL(statement, true);
			if(page.pageID == -1)
				return ;
			statement = "update http_requests set page_id= "+page.pageID+" where id="+this.id;
			this.DBService.executeSQL(statement, true);
		//	dump("Finish updating request "+this.id+" \n");
			
		}
	},

	//also responsible for updating request's page field
	setPageInfo: function(page, newRequest){
		if(newRequest){
			this.PageInfo = page;
			this.StorePageToDB(page);
		}
		else if(!(this.PageInfo)){
		//	dump("Error setting pageinfo "+this.id+" "+newRequest+" "+this.PageInfo+"\n");
			this.PageInfo = page;
			this.StorePageToDB(page);
		}
		else if((this.PageInfo.pageID != page.pageID) && (page.pageID != -1) ){
		//	dump("Request "+this.id+" should be updated from "+
		//		this.PageInfo.pageID+" to "+page.pageID+"\n");
			this.PageInfo = page;
			this.StorePageToDB(page);
		}
	},
	...
	checkRequestState: function()
	{
			...
			&& (this.EventSourceData["flags"] & Components.interfaces.nsIWebProgressListener.STATE_REDIRECTING)) 
		{
			try{
				...
				dump("Error in getting respnse status : "+e+"\n");
			}
			this.setFinished();
	...
	{
		if(requestEvent.isEffectivePageInfo()){
		//	dump("updateFromRequestEvent "+ requestEvent.PageInfo.pageID+" "+requestEvent.Url+"\n");
			var newPage = new HttpFoxPage(requestEvent.PageInfo.pageID, 
										requestEvent.PageInfo.parentID,
										requestEvent.PageInfo.location);
			this.setPageInfo(newPage,false);
		}
	/*	else{
			if(!this.PageInfo){
				dump("updateFromRequestEvent this.PageInfo=null\n");
			}
			else{
				dump("updateFromRequestEvent No Update: ");
				dump("PageID1: "+this.PageInfo.pageID+
			 		" ParentID:"+this.PageInfo.parentID+
			 		" location:"+this.PageInfo.location+"\n");
			}
	...
	*/
		...
		/*	for(var x in requestEvent.RequestHeaders ){
				dump("DEBUG: "+x+"  "+requestEvent[x]+"\n");
			}
		*/
		}
...
function HttpFoxRequestEvent(HttpFoxReference, HttpChannelReference, EventSourceType, EventSourceMiscData, HttpFoxContext)
{
	try 
	{
		this.RequestChannel = HttpChannelReference;
		this.HttpChannel = HttpChannelReference.QueryInterface(Components.interfaces.nsIHttpChannel);
	}
	catch(ex) 
	{
		// discard that non-httpchannel thing
		//dump("Error in HttpFoxRequestEvent: "+ex);
		return
	}
...
HttpFoxObserver.prototype =
{
	...
	pageIDFromWindow : function (window) {
		try {
			return window.QueryInterface(Components.interfaces.nsIInterfaceRequestor).getInterface(Components.interfaces.nsIDOMWindowUtils).currentInnerWindowID;
		}catch(error) {
		}
		return -1;
	},

	pageIDFromHttpChannel : function(httpChannel) {
		try {
			...
			if(notificationCallbacks) {
				var loadContext = notificationCallbacks.getInterface(Components.interfaces.nsILoadContext)
				//dump("pageIDFromHttpChannel"+loadContext+"\n");
				var window = loadContext.associatedWindow;

				var pageID = this.pageIDFromWindow(window);
				var parentID = window.parent ? this.pageIDFromWindow(window.parent) : -1;
				var location = window.document && window.document.location ? window.document.location : "";
				var result = new HttpFoxPage(pageID, parentID, location);
				return result;
			}
		}
		catch(error) {
			//dump("Error getting page ID: " + httpChannel.URI.spec+"===="+error+"\n");
		}
		return new HttpFoxPage(-1, -1, "");
	},
	...
	onModifyRequest: function(HttpChannel)
	{
		...
		//Notice that Not all nsIRequest is nsIHttpChannel
		//For example, a request for a image is an imageRequest, which is also a nsIRequest
		//But not a nsIHttpChannel
		...
		try {
			...
			event.setPageInfo(page);
			//dump("window2: "+this.pageIDFromHttpChannel(HttpChannel)+"\n");
